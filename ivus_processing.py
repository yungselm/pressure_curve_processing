...
df_rest = pd.read_csv('C:/WorkingData/Documents/2_Coding/Python/pressure_curve_processing/000_Reports/NARCO_10_rest.txt', sep = '\t')
df_dobu = pd.read_csv('C:/WorkingData/Documents/2_Coding/Python/pressure_curve_processing/000_Reports/NARCO_10_stress.txt', sep = '\t')
...
def fit_curves_sys_dia(df, degree=10):
    ...
    df['fitted_lumen_area'] = df.groupby('phase', group_keys=False).apply(lambda group: pd.Series(polynomial_fit(group['distance'], group['lumen_area'], degree), index=group.index))
...
def fit_curve_global(df, degree=3):
    """ Fits a polynomial curve to the mean lumen area and elliptic ratio. """
    df['mean_lumen_area'] = df.groupby('distance')['lumen_area'].transform('mean')
    df['fitted_mean_lumen_area'] = polynomial_fit(df['distance'], df['mean_lumen_area'], degree)
    df['mean_elliptic_ratio'] = df.groupby('distance')['elliptic_ratio'].transform('mean')
    df['fitted_mean_elliptic_ratio'] = polynomial_fit(df['distance'], df['mean_elliptic_ratio'], degree)
    return df
...
# Apply polynomial fitting to the lumen_area for each phase separately
df_rest['fitted_lumen_area'] = df_rest.groupby('phase', group_keys=False).apply(lambda group: pd.Series(polynomial_fit(group['distance'], group['lumen_area']), index=group.index))
df_dobu['fitted_lumen_area'] = df_dobu.groupby('phase', group_keys=False).apply(lambda group: pd.Series(polynomial_fit(group['distance'], group['lumen_area']), index=group.index))

# Calculate the mean elliptic_ratio for systole and diastole
df_rest['mean_elliptic_ratio'] = df_rest.groupby('distance')['elliptic_ratio'].transform('mean')
df_dobu['mean_elliptic_ratio'] = df_dobu.groupby('distance')['elliptic_ratio'].transform('mean')

# Apply polynomial fitting to the mean elliptic_ratio
df_rest['fitted_elliptic_ratio'] = polynomial_fit(df_rest['distance'], df_rest['mean_elliptic_ratio'])
df_dobu['fitted_elliptic_ratio'] = polynomial_fit(df_dobu['distance'], df_dobu['mean_elliptic_ratio'])

# order df_rest and df_dobu by distance
df_rest = df_rest.sort_values(by='distance')
df_dobu = df_dobu.sort_values(by='distance')
...
ax1.text(min_lumen_area_distance_rest, min_lumen_area_rest, f'{min_lumen_area_rest:.2f} ({min_lumen_area_frame_rest})', color='#0055ff')

# Overlay mean elliptic ratio for rest
ax1.plot(df_rest['distance'], df_rest['fitted_elliptic_ratio'], color='darkblue', label='Mean Elliptic Ratio - Rest')

ax1.set_xlim(x_min, x_max)
ax1.set_ylim(min(y_min_lumen, y_min_elliptic), max(y_max_lumen, y_max_elliptic))
ax1.set_xlabel('Distance (mm)')
ax1.set_ylabel('Lumen Area (mm²) / Elliptic Ratio')
...
ax2.text(min_lumen_area_distance_dobu, min_lumen_area_dobu, f'{min_lumen_area_dobu:.2f} ({min_lumen_area_frame_dobu})', color='#0055ff')

# Overlay mean elliptic ratio for dobutamine
ax2.plot(df_dobu['distance'], df_dobu['fitted_elliptic_ratio'], color='darkblue', label='Mean Elliptic Ratio - Dobutamine')

ax2.set_xlim(x_min, x_max)
ax2.set_ylim(min(y_min_lumen, y_min_elliptic), max(y_max_lumen, y_max_elliptic))
ax2.set_xlabel('Distance (mm)')
ax2.set_ylabel('Lumen Area (mm²) / Elliptic Ratio')
